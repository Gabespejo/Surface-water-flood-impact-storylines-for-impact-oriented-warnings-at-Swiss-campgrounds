...
from flow_depth_plotting import compare_manning_difference_noabsolute
...
def main():
    ...
    parser.add_argument("--version-a", default="", help="Version suffix for Solver A (e.g. _v1, _v2, _v3, _v4 or empty)")
    parser.add_argument("--version-b", default="", help="Version suffix for Solver B (e.g. _v1, _v2, _v3, _v4 or empty)")
    parser.add_argument("--duration-a", required=True, help="Simulation duration for Solver A (e.g. 60min)")
    parser.add_argument("--duration-b", required=True, help="Simulation duration for Solver B (e.g. 75min)")
    ...
    for rain in range(args.start, args.end + 1, args.step):
        ...
        fname_a = f"{base_name_a}_{args.duration_a}_{rain}.max"
        fname_b = f"{base_name_b}_{args.duration_b}_{rain}.max"

        fpath_a = os.path.join(args.base_dir_a, folder_a, fname_a)
        fpath_b = os.path.join(args.base_dir_b, folder_b, fname_b)

        outname = f"{base_name_a}_vs_{base_name_b}_diff_{rain}mm.png"
        ...
        compare_manning_difference_noabsolute(
