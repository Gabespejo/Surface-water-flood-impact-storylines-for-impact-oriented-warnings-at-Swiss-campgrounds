...
def main():
    ...
    with rasterio.open(dem_path) as src:
        ...
        print(f"📐 Loaded DEM bounding box from raster: {xmin}, {ymin}, {xmax}, {ymax}")

    # Load Arealstatistik and map LC → Manning
    gdf = gpd.read_file(args.areal_gpkg)
    target_field = args.code_field + "_manning"
    gdf[target_field] = gdf[args.code_field].map(MANNING)

    # Use consistent areal-prefixed intermediate names
    ...
    # Step 3: Resample
    print(" Resampling to 2 m resolution...")
    resample_raster(clipped, tif2m, args.res_2m, resampling_method="nearest")

    # Step 4: Convert to .asc and rename
    ...
    # Step 6: Cleanup
    for tmp in [tif100, clipped, tif2m, asc]:
